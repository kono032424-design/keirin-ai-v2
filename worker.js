...
async function autoRun(){
  ...
  const todayResp=await apiToday(new URL(`https://local/api/today?date=${date}`)),today=await todayResp.json();
  const hour=jst.getUTCHours(),minute=jst.getUTCMinutes();
  // 10:00を1Rの目安に30分刻みで対象を進め、直近2Rだけ確認する。1〜12Rに制限。
  const base=Math.max(1,Math.min(12,Math.floor(((hour*60+minute)-(10*60))/30)+1));
  const races=[base,Math.min(12,base+1)].filter((x,i,a)=>a.indexOf(x)===i),out=[];
  for(const v of (today.venues||[]))for(const race of races){
    const raceId=date+v.code+String(race).padStart(2,'0');
    try{const er=await apiEntry(new URL(`https://local/api/entry?race_id=${raceId}`));if(!er.ok)continue;const e=await er.json();const pred=serverPredict(e.riders,e.lineText);out.push({raceId,track:v.name,race,lineText:e.lineText||'',...pred});}catch(_){ }
  }
  const payload={ok:true,ranAt:new Date().toISOString(),date,targetRaces:races,predictions:out};
  try{const cache=caches.default,key=new Request('https://keirin-ai-v2.local/__auto_latest');await cache.put(key,new Response(JSON.stringify(payload),{headers:{'content-type':'application/json','cache-control':'public,max-age=21600'}}));}catch(_){ }
  console.log('AUTO_RUN',JSON.stringify(payload));return payload;
}
async function apiAutoStatus(){
  try{const r=await caches.default.match(new Request('https://keirin-ai-v2.local/__auto_latest'));if(r)return new Response(r.body,{headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store'}})}catch(_){ }
