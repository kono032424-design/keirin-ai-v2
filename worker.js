...
function parseRiders(rows){
  ...
  for(const row of rows){
    ...
    for(let i=0;i<c.length-1;i++){
      ...
      if(/^\d{2,3}\.\d{1,2}$/.test(c[i])&&score>=50&&score<=150&&/^\d*[逃追両]$/.test(c[i+1]||'')){si=i;break}
    }
    if(si<1)continue;
    ...
    let car=0;
    for(let i=0;i<si;i++){if(/^[1-9]$/.test(c[i]))car=+c[i]}
    if(!car)continue;
...
async function apiEntry(url){
  ...
  if(riders.length<5){
    const debugRows=rows.slice(0,25).map(row=>row.map(x=>txt(x.v)));
    return json({
      error:`出走表を解析できませんでした（${riders.length}車取得）。`,
      race_id:id,
      source:src,
      debug:{
