...
const AUTO_LATEST_KEY=AUTO_LATEST_KEY;
async function readAutoState(){try{const r=await caches.default.match(AUTO_STATE_KEY);if(r)return await r.json()}catch(_){}return null}
async function writeAutoState(v){try{await caches.default.put(AUTO_STATE_KEY,new Response(JSON.stringify(v),{headers:{'content-type':'application/json','cache-control':'public,max-age=86400'}}))}catch(_){}}
async function writeAutoLatest(v){try{await caches.default.put(AUTO_LATEST_KEY,new Response(JSON.stringify(v),{headers:{'content-type':'application/json','cache-control':'public,max-age=21600'}}))}catch(_){}}
async function autoRun(){
  ...
  if(st.phase==='entry'||!st.pending||st.pending.raceId!==raceId){
    try{const riders=await autoFetchEntryRiders(raceId);st.pending={raceId,track:v.name,race,riders};st.phase='line';st.updatedAt=new Date().toISOString();await writeAutoState(st);console.log('AUTO_STAGE entry',raceId,riders.length)}
    catch(e){st.venueIndex++;st.phase='entry';st.pending=null;st.updatedAt=new Date().toISOString();await writeAutoState(st);console.log('AUTO_SKIP entry',raceId,e?.message||String(e))}
    return;
  }
  try{
    const lineText=await autoFetchLine(raceId,st.pending.riders),pred=serverPredict(st.pending.riders,lineText);
    const item={raceId,track:v.name,race,lineText,...pred,at:new Date().toISOString()};
    st.predictions=(st.predictions||[]).filter(x=>x.raceId!==raceId);st.predictions.push(item);
    const payload={ok:true,ranAt:new Date().toISOString(),date,predictions:st.predictions.slice(-20)};
    await writeAutoLatest(payload);console.log('AUTO_STAGE predict',JSON.stringify(item));
  }catch(e){console.log('AUTO_SKIP line',raceId,e?.message||String(e))}
  st.venueIndex++;st.phase='entry';st.pending=null;st.updatedAt=new Date().toISOString();await writeAutoState(st);
}

async function apiAutoStatus(){
  try{const r=await caches.default.match(AUTO_LATEST_KEY);if(r)return new Response(r.body,{headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store'}})}catch(_){ }
  return json({ok:false,message:'まだ自動実行結果がありません。Cron実行後に確認してください。'});
}

export default {
  async scheduled(event, env, ctx) {
    ctx.waitUntil(autoRun());
  },
  async fetch(request) {
    ...
    try {
      ...
      if (url.pathname === "/api/auto-status") return await apiAutoStatus();
