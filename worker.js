...
function parseRiders(rows){
  ...
  for(const row of rows){
    ...
    // 実データ例:
    // ["1","1",...,"5","コバヤシタカシ 小林高志","68.48","3 追","0","0","0",...]
    // 車番は行先頭、競走得点の次は「3 追」「1 逃」「2 両」形式。
    const car=Number(c[0]);
    if(!Number.isInteger(car)||car<1||car>9)continue;

    let si=-1;
    for(let i=1;i<c.length-1;i++){
      const score=Number(c[i]);
      if(/^\d{2,3}\.\d{1,2}$/.test(c[i]) && score>=50 && score<=150 &&
         /^\d+\s*[逃追両]$/.test(c[i+1]||'')){
        si=i; break;
      }
    }
    ...
    let name=txt(c[si-1]||'');
    // 読み仮名＋漢字名が同一セルの場合、末尾の漢字名を優先。
    const kanji=name.match(/([一-龯々ヶヵ\s]{2,})$/);
    if(kanji)name=txt(kanji[1]);
    if(!name||name.length>30)name='車番'+car;

    const n=(idx)=>{
      const m=String(c[idx]||'').match(/-?\d+(?:\.\d+)?/);
    ...
    // 勝率・2連対率・3連対率は、競走得点より後ろにある最初の3つの % 値。
    const pcts=c.slice(si+2)
      .filter(v=>/^\d+(?:\.\d+)?%$/.test(v))
      .slice(0,3)
      .map(v=>parseFloat(v));
  ...
  const dedup=new Map();
  for(const x of out)if(!dedup.has(x.num))dedup.set(x.num,x);
