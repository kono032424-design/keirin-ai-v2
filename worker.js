...
function serverPredict(riders,lineText){
  ...
  const groups=String(lineText||'').split('/').map(x=>x.trim()).filter(Boolean).map(x=>x.split('-').map(Number).filter(Boolean)),pos={};
  groups.forEach((g,gi)=>g.forEach((n,i)=>pos[n]={g:gi,i,len:g.length}));
  ...
  R.forEach(x=>{const attack=x.style==='逃'?1:x.style==='両'?.6:.15;let sc=34*nrm(C.pts,x.pts)+18*nrm(C.win,x.win)+14*nrm(C.top2,x.top2)+10*nrm(C.top3,x.top3)+8*nrm(C.b,x.b)+4*nrm(C.s,x.s)+4*nrm(C.h,x.h)+8*attack;const p=pos[x.num];if(p){if(p.i===0&&(x.style==='逃'||x.style==='両'))sc+=4.3;if(p.i===1)sc+=4;if(p.i>=2)sc+=1.3;if(p.len>=3)sc+=1}x.ai=sc});
  const mx=Math.max(...R.map(x=>x.ai)),T=13,E=R.map(x=>Math.exp((x.ai-mx)/T)),sum=E.reduce((a,b)=>a+b,0);R.forEach((x,i)=>x.p=E[i]/sum);
  const rank=[...R].sort((a,b)=>b.ai-a.ai),comb=[];
  R.forEach(a=>R.forEach(b=>R.forEach(c=>{if(new Set([a.num,b.num,c.num]).size<3)return;let p=a.p*(b.p/(1-a.p))*(c.p/(1-a.p-b.p));const A=pos[a.num],B=pos[b.num],C2=pos[c.num];if(A&&B&&A.g===B.g)p*=1.15;if(B&&C2&&B.g===C2.g)p*=1.08;if(A&&B&&C2&&A.g===B.g&&B.g===C2.g)p*=1.08;comb.push({a:a.num,b:b.num,c:c.num,p})})));
  const sp=comb.reduce((a,b)=>a+b.p,0);comb.forEach(x=>x.p/=sp);comb.sort((a,b)=>b.p-a.p);const top=comb.slice(0,6),focus=rank.slice(0,3).reduce((a,b)=>a+b.p,0),gap=rank[0].ai-rank[1].ai;let grade='B';if(rank[0].p>=.28&&gap>=6&&focus>=.68)grade='S';else if(rank[0].p>=.21&&gap>=3&&focus>=.60)grade='A';
  return {grade,main:{num:rank[0].num,name:rank[0].name,p:rank[0].p},focus,gap,top:top.map(x=>({a:x.a,b:x.b,c:x.c,p:x.p}))};
}
...
async function autoRun(env){
  const now=new Date(),jst=new Date(now.getTime()+9*3600000),date=`${jst.getUTCFullYear()}${String(jst.getUTCMonth()+1).padStart(2,'0')}${String(jst.getUTCDate()).padStart(2,'0')}`;
  let st=await readAutoState(env);
  // CPU上限対策: 1回のCronでは外部ページを1枚だけ解析する。
  if(!st||st.date!==date||!Array.isArray(st.venues)||!st.venues.length){
    ...
    st={date,venues:tj.venues||[],venueIndex:0,phase:'entry',pending:null,predictions:[],updatedAt:new Date().toISOString()};
    await writeAutoState(env,st);
    console.log('AUTO_STAGE calendar',JSON.stringify({date,venues:st.venues.length}));
    return;
  }
  if(!st.venues.length)return;
  if(st.venueIndex>=st.venues.length){st.venueIndex=0}
  const v=st.venues[st.venueIndex];
  const mins=jst.getUTCHours()*60+jst.getUTCMinutes();
  const race=Math.max(1,Math.min(12,Math.floor((mins-600)/30)+1));
  const raceId=date+v.code+String(race).padStart(2,'0');
  if(st.phase==='entry'||!st.pending||st.pending.raceId!==raceId){
    try{const riders=await autoFetchEntryRiders(raceId);st.pending={raceId,track:v.name,race,riders};st.phase='line';st.updatedAt=new Date().toISOString();await writeAutoState(env,st);console.log('AUTO_STAGE entry',raceId,riders.length)}
    catch(e){st.venueIndex++;st.phase='entry';st.pending=null;st.updatedAt=new Date().toISOString();await writeAutoState(env,st);console.log('AUTO_SKIP entry',raceId,e?.message||String(e))}
    return;
  }
  try{
    const lineText=await autoFetchLine(raceId,st.pending.riders),pred=serverPredict(st.pending.riders,lineText);
    const item={raceId,date,track:v.name,race,lineText,...pred,at:new Date().toISOString()};
    st.predictions=(st.predictions||[]).filter(x=>x.raceId!==raceId);st.predictions.push(item);st.predictions=st.predictions.slice(-50);
    const payload={ok:true,ranAt:new Date().toISOString(),date,predictions:st.predictions};
    await writeAutoLatest(env,payload);console.log('AUTO_STAGE predict',JSON.stringify(item));
  }catch(e){console.log('AUTO_SKIP line',raceId,e?.message||String(e))}
  st.venueIndex++;st.phase='entry';st.pending=null;st.updatedAt=new Date().toISOString();await writeAutoState(env,st);
}

async function apiAutoStatus(env){
  const latest=await readAutoLatest(env);
  if(latest)return json(latest);
  const st=await readAutoState(env);
  return json({ok:false,message:'まだ自動予想がありません。Cronの処理後に確認してください。',state:st?{date:st.date,venueIndex:st.venueIndex,phase:st.phase,updatedAt:st.updatedAt}:null,predictions:[]});
}
