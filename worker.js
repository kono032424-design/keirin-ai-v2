...
function parseRiders(rows){
  ...
  for(const row of rows){
    ...
    out.push({num:car,name,score:parseFloat(c[si]),style:st,s,h,b,escape,makuri,win,top2,top3});
  }
...
function parseLineText(bodyText,riders){
  ...
  const marker='並び予想';
  const a=all.indexOf(marker);
  if(a<0)return '';
  // Keep only the small section after 並び予想 so unrelated race numbers are ignored.
  const sec=all.slice(a+marker.length,a+marker.length+700);
  const nums=[];
  const names=new Map(riders.map(x=>[x.num,String(x.name||'').replace(/\s/g,'')]));
  // netkeirin displays each line as consecutive "車番+選手名" blocks.
  // Text extraction loses visual gaps, so use rider names as anchors and infer groups
  // from the displayed order plus rider comments is unsafe. We only auto-fill when
  // explicit separators are recoverable; otherwise leave blank rather than guess.
  for(const m of sec.matchAll(/(?:^|\s)([1-9])(?=[^0-9%]{1,16})/g)){
    const n=+m[1]; if(names.has(n)&&!nums.includes(n))nums.push(n);
  }
  // A plain sequence alone does not encode line boundaries reliably.
  return '';
}
